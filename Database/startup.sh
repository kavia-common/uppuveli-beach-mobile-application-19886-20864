...
# Load env from .env if present (supports lines like KEY=VALUE and export KEY=VALUE)
if [ -f ".env" ]; then
  # shellcheck disable=SC2046
  export $(grep -v '^\s*#' .env | sed -e '/^\s*$/d' -e 's/^export\s\+//' | xargs -I{} echo {})
fi

# Minimal PostgreSQL startup script with .env support and sane defaults
# Harmonized default port to 5001 while allowing overrides via .env/ENV
...
echo "  POSTGRES_PORT=${DB_PORT} (default 5001; override via .env/ENV)"
...
if sudo -u postgres "${PG_BIN}/pg_isready" -p "${DB_PORT}" > /dev/null 2>&1; then
  ...
  # Update connection info file to reflect current env
  echo "psql postgresql://${DB_USER}:${DB_PASSWORD}@localhost:${DB_PORT}/${DB_NAME}" > db_connection.txt
  echo "Or use: $(cat db_connection.txt)"

  # Update db_visualizer env to remain compatible with helper scripts
...
# Save connection command to a file (CLI connection string)
echo "psql postgresql://${DB_USER}:${DB_PASSWORD}@localhost:${DB_PORT}/${DB_NAME}" > db_connection.txt
echo "Connection string saved to db_connection.txt"

# Save environment variables to a file for db_visualizer tool (compatibility)
mkdir -p db_visualizer
cat > db_visualizer/postgres.env << EOF
export POSTGRES_URL="postgresql://localhost:${DB_PORT}/${DB_NAME}"
export POSTGRES_USER="${DB_USER}"
